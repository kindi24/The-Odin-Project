...
WHERE w1.name = (SELECT MIN(w2.name) FROM world w2 WHERE w1.continent = w2.continent)
...
WHERE (SELECT MAX(w2.population) FROM world w2 WHERE w1.continent = w2.continent)<= 25000000

-- 10. Some countries have populations more than three times that of all of their neighbours (in the same continent). Give the countries and continents
SELECT w1.name, w1.continent FROM world w1
WHERE w1.population/3 > ALL(SELECT w2.population FROM world w2 WHERE w1.continent = w2.continent AND w1.name <> w2.name)